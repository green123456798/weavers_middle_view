		...
		<!-- 리뷰를 작성한 상품일 경우 작성하기 버튼 -->
			<c:if test="${data.rpk != 0}">
				...
				<div class="form-group">
					...
					<!-- 구매한 상품 리뷰에 등록할 이미지 -->
						<img id="preview" style="max-width: 200%; max-height: 200px;"
							class="img-fluid" src="uploadimg/${data.img}" alt="업로드할 이미지"> <input type="file" 
							name="file" onchange="readURL(this);" value="${data.img}"> 
						<input type="hidden" name="prevImg" value= "${data.img}"> <!-- 이미지 미리보기 -->
						
						<input type="hidden" name="scope" id="scope" value="">
						<c:if test="${data.scope == 0}">
						<star:star id="myStarRating" defaultRating="1" />	<!-- 별점이 등록되지 않았으면 기본값은 별1개 -->
						</c:if>
						<c:if test="${data.scope > 0}">			
				...
				<div style="display: flex; justify-content: flex-end;">
					...
					<a href="checkoutList.do"									
						class="button button-postComment button--active" 
						style="margin-left: 20px"> 취 소</a>			<!-- 취소 버튼을 누르면 이전페이지인 구매내역 페이지로 이동 -->
				</div>
			</form>
